...
const NavbarComponentBgOrange = () => {
  ...
  return (
    <nav
      className={`shadow-md bg-orange-500 fixed top-0 w-full z-50 transition-all duration-300 ${
    ...
    >
      ...
      {dropDown && (
        <div>
          <div
            className={`sm:hidden h-auto shadow-xl bg-orange-500`}
            id="mobile-menu"
          >
              ...
              {navLinks.map((index) => {
                ...
                return (
                  <Link
                    key={index.id}
                    to={index.path}
                    className={`block rounded-full px-3 py-2 text-base font-medium text-white
                            ${isActive ? "bg-white/10 backdrop-blur-md" : ""}
                            hover:bg-white/10 hover:backdrop-blur-md hover:text-white`}
                  >
                    {index.name}
                  </Link>
                );
...
export default NavbarComponentBgOrange;
